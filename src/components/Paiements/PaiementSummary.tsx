      ...
      <div className="flex items-center space-x-4 text-sm">
        ...
        {excludeAnnules && statsAnnules.total > 0 && (
          <span className="text-gray-600 font-medium">
            {statsAnnules.total} annulé{statsAnnules.total !== 1 ? 's' : ''}
          </span>
        )}
      </div>
    );
    ...
    <div className="space-y-3">
      ...
      {excludeAnnules && statsAnnules.total > 0 && (
        <div className="text-xs text-red-600 bg-red-50 px-2 py-1 rounded">
          {statsAnnules.total} volontaire{statsAnnules.total !== 1 ? 's' : ''} annulé{statsAnnules.total !== 1 ? 's' : ''} 
          exclu{statsAnnules.total !== 1 ? 's' : ''} ({statsAnnules.montantTotal.toFixed(0)}€)
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="text-center p-2 bg-gray-50 rounded">
          ...
          <div className="text-xs text-gray-600">Total{excludeAnnules ? ' actifs' : ''}</div>
      ...
      {excludeAnnules && statsAnnules.total > 0 && (
        <div className="border-t pt-2">
          <div className="text-center p-2 bg-gray-100 rounded">
            ...
            <div className="text-xs text-gray-600">Annulés (exclus)</div>
