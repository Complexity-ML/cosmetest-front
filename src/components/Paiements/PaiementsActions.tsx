...
const PaiementActions = ({
  ...
  const handleStatusChange = async (nouveauStatut: number) => {
    ...
    if (isAnnule) {
      setError(t('payments.cannotModifyCancelledPayment'));
      return;
    }
    ...
    try {
      ...
      setError(t('payments.updateError'));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        {Object.entries(PAIEMENT_STATUS).map(([value, config]) => {
          ...
          return (
            <button
              ...
              `}
              title={isAnnule ? t('payments.volunteerCancelledBlocked') : ''}
            >
    ...
    <div className="relative">
      ...
      {montant > 0 && (
        <div className={`mt-1 text-xs text-gray-600 text-center ${isAnnule ? 'line-through' : ''}`}>
          {montant.toFixed(0)} €
          {isAnnule && <span className="block text-red-600">({t('payments.excluded')})</span>}
        </div>
      )}
