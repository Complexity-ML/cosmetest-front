...
const SPECIFIC_EVALUATIONS = [
  { name: "evaluationYeux", label: "Yeux" },
  { name: "evaluationLevres", label: "Levres" },
  { name: "evaluationTeint", label: "Teint" },
  { name: "evaluationCynetique", label: "Cynetique" },
];
...
const EvaluationSection = ({ formData, errors, onChange }: any) => {
  ...
  const renderStarRow = (fieldName: string, label: string, size = "md") => {
    ...
    const ariaSuffix = label ? ` pour ${label}` : "";
    ...
    return (
      <div className="flex items-center">
          ...
          {STAR_VALUES.map((star) => {
            ...
            return (
              <button
                ...
                aria-label={`${star} etoile${star > 1 ? "s" : ""}${ariaSuffix}`}
              >
                <svg
        ...
        <span className="ml-3 text-sm text-gray-600">
          {currentValue}/{STAR_VALUES.length} etoile{currentValue > 1 ? "s" : ""}
        </span>
      </div>
    );
  };
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Evaluation du volontaire</CardTitle>
      </CardHeader>
      <CardContent>

      <div className="space-y-8">
        <div>
          <p className="text-sm font-medium text-gray-700 mb-2">Evaluation globale</p>
          ...
          >
            Reinitialiser la note
          </button>
        </div>

        <div>
          <p className="text-sm font-medium text-gray-700 mb-4">Evaluation des zones specifiques</p>
